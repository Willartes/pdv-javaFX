...
public class MainController implements Initializable {
 ...
 // Lista observável para armazenar os itens do pedido atual
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Opcional: Também buscar quando o campo perde o foco
    ...
    /**
     * Busca um produto pelo código ou nome e preenche os campos
     */
    private void buscarProdutoPorCodigo() {
        String codigoOuNome = productField.getText().trim();
        
        if (codigoOuNome.isEmpty()) {
            return; // Não fazer nada se o campo estiver vazio
        }
        ...
        try {
            ...
            // Tentar buscar primeiro por código exato
            produto = produtoDAO.findByCodigo(codigoOuNome);
            
            // Se não encontrar por código, tentar por nome
            if (produto == null) {
                List<Produto> produtos = produtoDAO.findByNome(codigoOuNome);
                if (!produtos.isEmpty()) {
                    produto = produtos.get(0);
                }
            }
            
            // Se encontrou o produto, preencher os campos
            if (produto != null) {
                selecionarProduto(produto);
            } else {
                // Produto não encontrado - perguntar se deseja abrir tela de produtos
                boolean abrirTela = AlertUtil.showConfirmation(
                    "Produto não encontrado", 
                    "O produto '" + codigoOuNome + "' não foi encontrado. Deseja abrir a tela de produtos?");
    ...
    */
    ...
    public void abrirTelaProdutos() {
        try {
            // Caminho correto para o arquivo FXML de produtos
            ...
            Scene produtosScene = new Scene(root, width, height);
 ...
 // Modifique o método abrirTelaRelatorios() na classe MainController
    ...
    public void abrirTelaRelatorios() {
        try {
            ...
            // ADICIONE ESTA LINHA: Passar a referência da cena principal
            ...
            AlertUtil.showError("Erro ao abrir relatórios", "Não foi possível abrir a tela de relatórios: " + e.getMessage());
        }
    }
 ...
 // Código para substituir o botão verde por uma barra de busca e botão de menu
    ...
    private void realizarBusca(String termo) {
        // Declarar o DAO fora do bloco try para que esteja acessível em todo o método
        ProdutoDAO produtoDAO = ProdutoDAO.getInstance();
        ...
        try {
            ...
            if (termo.isEmpty()) {
                ...
                if (termo.matches("\\d+")) {
                    // Correção: usar findById com a conexão padrão
                	Produto produto = produtoDAO.findById(Integer.parseInt(termo));
                    produtos = produto != null ? List.of(produto) : new ArrayList<>();
                } else {
                    // Verificar se pode ser um código de produto não numérico
                    Produto produtoPorCodigo = produtoDAO.findByCodigo(termo);
                    if (produtoPorCodigo != null) {
                        produtos = List.of(produtoPorCodigo);
                    } else {
            ...
            // Caso o parse para Integer falhe, tenta buscar por nome
            try {
                List<Produto> produtos = produtoDAO.findByNome(termo);
                mostrarResultadosBusca(produtos, termo);
            } catch (SQLException ex) {
                AlertUtil.showError("Erro ao buscar produtos", ex.getMessage());
            }
        }
    }
    /**
